<?php
    ...
    $playlist_sql = "SELECT Name FROM playlists WHERE PlaylistId = ?";
	  $statement = $pdo->prepare($sql);
    $statement->bindParam(1, $_GET['playlist']);
    $statement->execute();
    $tracks = $statement->fetchAll(PDO::FETCH_OBJ);
    $statement2 = $pdo->prepare($playlist_sql);
    $statement2->bindParam(1, $_GET['playlist']);
    $statement2->execute();
    $playlist = $statement2->fetchAll(PDO::FETCH_OBJ);
?>
<?php if(sizeof($tracks) == 0):?>
          <p>No tracks found for <?php echo $playlist[0]->Name ?></p>
<?php else: ?>
<table>
  <thead>
    <th>Track name</th>
